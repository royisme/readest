...
const convertTxtToEpubInWorker = async (options: ConvertTxtToEpubOptions) => {
  ...
  return await new Promise<Awaited<ReturnType<TxtToEpubConverter['convert']>>>((resolve, reject) => {
    ...
    worker.onmessage = (event: MessageEvent<TxtConverterWorkerResponse>) => {
      ...
      const { epubBuffer, name, bookTitle, chapterCount, language: detectedLanguage } =
        event.data.payload;
      cleanup();
      resolve({
