...
const fetchWithOptionalAuthMock = vi.fn();

vi.mock('@/utils/fetch', () => ({
  fetchWithOptionalAuth: (...args: unknown[]) => fetchWithOptionalAuthMock(...args),
}));
...
describe('RemoteTTSClient', () => {
  beforeEach(() => {
    fetchWithOptionalAuthMock.mockReset();
    fetchWithOptionalAuthMock.mockResolvedValue(
  ...
  it('should keep end event semantics for auto-next-chapter flow', async () => {
    ...
    expect(fetchWithOptionalAuthMock).toHaveBeenCalledTimes(1);
  ...
  it('should prefetch upcoming segments without breaking end event', async () => {
    ...
    let inFlight = 0;
    let maxInFlight = 0;
    fetchWithOptionalAuthMock.mockImplementation(async () => {
      inFlight += 1;
  ...
  it('should prefetch upcoming streamed segments', async () => {
    const dispatchSpeakMark = vi.fn();
    const settings: TTSSettings = {
      defaultEngine: 'remote-tts',
      activeProviderId: 'provider-1',
