...
This crate provides Windows Explorer thumbnail support for eBook files when Inkline is set as the default application.

## Features

- **Automatic Cover Extraction**: Extracts cover images from EPUB, MOBI, AZW, AZW3, FB2, CBZ, CBR files
- **Inkline Branding**: Adds a small Inkline icon overlay at the bottom-right corner
- **Smart Caching**: Caches generated thumbnails for faster subsequent loads
- **File Association Aware**: Only shows thumbnails when Inkline is the default app for the file type
...
The thumbnail provider DLL is automatically registered when Inkline is installed via the NSIS installer.
...
For local development and testing, build the Windows DLL (or the library) from the Inkline Tauri app folder and register it manually. The legacy CLI test harness used to live in the separate `packages/tauri` workspace, but the thumbnail handler implementation now lives inside Inkline's Tauri app.
...
This generates a thumbnail with the Inkline overlay at the specified size.
...
3. It checks if Inkline.exe is the default application for that file type using `AssocQueryStringW`
4. If Inkline is the default, it extracts the cover and generates the thumbnail
5. If Inkline is NOT the default, it returns `S_FALSE` to let Windows use other handlers

This ensures thumbnails only appear for files the user has associated with Inkline.
